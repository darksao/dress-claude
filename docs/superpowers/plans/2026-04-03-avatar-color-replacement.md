# Avatar Color Replacement Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers-extended-cc:subagent-driven-development (recommended) or superpowers-extended-cc:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace SVG overlay approach in AvatarSvg.tsx with canvas-based pixel recoloration that preserves 3D shading while changing clothing colors to exactly the user's chosen hue.

**Architecture:** Load PNG into offscreen canvas, compute per-pixel zone masks (HSL classification), then on each color change iterate pixels to replace H+S while keeping L. SVG overlay stays invisible for click detection.

**Tech Stack:** Next.js 14, React, HTML Canvas API, TypeScript

**User Verification:** YES — user confirms avatar colors change correctly and 3D shading is preserved after implementation.
...
|------|--------|----------------|
| `src/components/avatar/AvatarSvg.tsx` | Modify | Canvas rendering + pixel recoloration + SVG click overlay |

---

### Task 1: Canvas rendering with pixel recoloration

**Goal:** Rewrite AvatarSvg.tsx to use canvas-based pixel recoloration — load PNG, compute zone masks, recolor on state change.

**Files:**
- Modify: `src/components/avatar/AvatarSvg.tsx`

**Acceptance Criteria:**
- [ ] Avatar renders the PNG via canvas (no visible difference from before on initial load)
- [ ] Clicking a zone and picking a color recolors that zone's pixels in the canvas
- [ ] 3D shading (luminosity variation) is preserved after recoloring
- [ ] Other zones and skin/background are unaffected
- [ ] SVG overlay zones remain clickable

**Verify:** Open http://localhost:3000/avatar, select a zone, pick a color → clothing changes color with shading visible.

**Steps:**

- [ ] **Step 1: Add HSL helper functions at top of file**
...
```bash
cd c:/Users/nhu-s/Documents/programs/dress-claude
npx tsc --noEmit
```
...
- Avatar renders with the PNG
- Click "Haut" zone → pick a color → jacket changes color
- 3D shading (lighter/darker areas) still visible
- Skin and background unaffected

- [ ] **Step 4: Commit**

```bash
git add src/components/avatar/AvatarSvg.tsx
git commit -m "feat: canvas pixel recoloration for avatar clothing zones"
```

---

### Task 2: User verification — colors and shading

**Goal:** Confirm the user is satisfied that colors change correctly and 3D effect is preserved.

**Files:** none

**Acceptance Criteria:**
- [ ] User confirms clothing color changes match their selection
- [ ] User confirms 3D shading is still visible

**Verify:** User feedback via AskUserQuestion.

**User Verification Required:**
Before marking this task complete, you MUST call AskUserQuestion:
```yaml
AskUserQuestion:
  question: "Teste l'avatar : clique sur une zone (ex: pantalon), choisis une couleur. Est-ce que (1) la couleur change bien et correspond à ce que tu as choisi, et (2) l'aspect 3D/relief est toujours visible ?"
  header: "Vérification avatar"
  options:
    - label: "Oui, les deux fonctionnent"
      description: "Couleur correcte + 3D préservé — implémentation validée"
    - label: "La couleur est mauvaise ou le 3D a disparu"
      description: "Besoin d'ajuster les plages HSL ou l'algorithme"
```

```json:metadata
{"files": [], "verifyCommand": "", "acceptanceCriteria": ["user confirms color matches selection", "user confirms 3D shading preserved"], "requiresUserVerification": true, "userVerificationPrompt": "Teste l'avatar : clique sur une zone, choisis une couleur. Est-ce que la couleur change correctement et le relief 3D est préservé ?"}
```
